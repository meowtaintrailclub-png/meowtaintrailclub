    ...
    <main style={{ maxWidth: 480, margin: "60px auto", textAlign: "center", fontFamily: "sans-serif" }}>
      ...
      {isEditing ? (
        <form action="/api/profile/update" method="POST" style={{ marginTop: 24, textAlign: "left", background: "#f7f7f7", padding: 20, borderRadius: 8 }}>
          ...
          <button type="submit" style={{ padding: "10px 20px", background: "#fc4c02", color: "white", border: "none", borderRadius: 6, fontWeight: 600, cursor: "pointer" }}>Save</button>
        ...
        <div style={{ marginTop: 24, textAlign: "left", background: "#f7f7f7", padding: 20, borderRadius: 8 }}>
          <h3 style={{ marginTop: 0 }}>My Details</h3>
          <p><strong>WhatsApp:</strong> {runner.whatsapp || "Not set"}</p>
          <p><strong>Address:</strong> {runner.address || "Not set"}</p>
          <a href="/profile?edit=1" style={{ display: "inline-block", marginTop: 8, padding: "8px 16px", background: "#fc4c02", color: "white", borderRadius: 6, textDecoration: "none", fontWeight: 600 }}>Edit</a>
        </div>
      )}

      <p style={{ marginTop: 32 }}><a href="/leaderboard">View this month's leaderboard →</a></p>
      <p style={{ marginTop: 16 }}><a href="/api/auth/logout" style={{ color: "#999" }}>Log out</a></p>
    </main>
  );
}
